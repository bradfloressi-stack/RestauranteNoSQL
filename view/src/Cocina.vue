    ...
    <header class="cocina-header">
      <div class="header-top">
        <div class="brand">
          ...
          <div class="user-pill">Cocina · {{ currentUser ? currentUser.nombre : '' }}</div>
        </div>
    ...
    <div class="content">
      <div class="columns">
        ...
        <div class="column">
          ...
          <div v-if="pendiente.length === 0" class="empty-msg">Sin platillos pendientes.</div>
          <div v-for="g in pendiente" :key="g.key" class="dish-card">
            ...
            <button class="btn-avanzar btn-avanzar-amber" @click="avanzarGrupo(g)">Iniciar preparación →</button>
          </div>
        </div>
        ...
        <div class="column">
          ...
          <div v-if="enProceso.length === 0" class="empty-msg">Nada cocinándose ahora.</div>
          <div v-for="g in enProceso" :key="g.key" class="dish-card">
            ...
            <div class="dish-nombre">{{ g.qty }}× {{ g.nombre }}</div>
            <div v-if="g.nota" class="dish-nota">Nota: {{ g.nota }}</div>
            <button class="btn-avanzar btn-avanzar-blue" @click="avanzarGrupo(g)">Enviar a ventanilla →</button>
          </div>
        </div>
        ...
        <div class="column">
          ...
          <div v-if="enVentanilla.length === 0" class="empty-msg">Nada esperando a que lo recojan.</div>
          <div v-for="g in enVentanilla" :key="g.key" class="dish-card dish-card-ready">
            <div class="dish-card-head">
              <div class="dish-mesa">Mesa {{ g.mesaNumero }}</div>
              <div class="dish-time">{{ formatTime(g.time) }}</div>
            </div>
            <div class="dish-nombre">{{ g.qty }}× {{ g.nombre }}</div>
            <div class="dish-esperando">Esperando a que el mesero lo recoja</div>
          </div>
        </div>
      </div>
    </div>
...
export default {
  ...
  computed: {
    ...
    gruposFlat() {
      ...
      this.ordenesActivas.forEach(o => {
        o.platillos.forEach(p => {
          ...
          if (!grupos[key]) {
            grupos[key] = {
              key, orderId: o._id, platilloId: p._id, ids: [],
              mesaNumero: o.mesa ? o.mesa.numero : '—', nombre: p.nombre, estado: p.estado,
              nota: o.nota, qty: 0, time: this.timestampDe(p._id),
            }
          }
          grupos[key].qty += 1
    ...
    pendiente() {
      return this.gruposFlat.filter(g => g.estado === 'proceso').sort((a, b) => a.time - b.time)
    },
    enProceso() {
      return this.gruposFlat.filter(g => g.estado === 'cocinado').sort((a, b) => a.time - b.time)
    },
    enVentanilla() {
      return this.gruposFlat.filter(g => g.estado === 'ventanilla').sort((a, b) => a.time - b.time)
    },
  },
  ...
  methods: {
    ...
    async avanzarGrupo(g) {
      try {
        await Promise.all(
          g.ids.map(id => this.peticion(`/ordenes/${g.orderId}/platillos/${id}/avanzar`, { method: 'PUT' }))
        )
...
/* ---------- header ---------- */
.cocina-header { background: #2f4d3a; color: #fff; }
...
.columns { display: grid; grid-template-columns: repeat(3, 1fr); gap: 18px; align-items: start; }
@media (max-width: 900px) { .columns { grid-template-columns: 1fr; } }

.column-head { display: flex; align-items: center; gap: 8px; margin-bottom: 12px; padding: 0 2px; }
.column-dot { width: 9px; height: 9px; border-radius: 999px; }
.dot-amber { background: #d99a1f; }
.dot-blue { background: #3d5a80; }
.dot-purple { background: #6b4f8f; }
.column-title { font-family: 'Instrument Serif', Georgia, serif; font-size: 18px; font-style: italic; color: #2f4d3a; }
.column-count { margin-left: auto; font-size: 12px; font-weight: 700; color: #9ca3af; background: #fff; border: 1px solid #ece5dd; padding: 2px 9px; border-radius: 999px; }

.empty-msg { padding: 24px 14px; text-align: center; color: #9ca3af; font-size: 13.5px; background: #fff; border-radius: 12px; border: 1px dashed #e2ddd4; }

.dish-card { background: #fff; border-radius: 12px; padding: 14px; border: 1px solid #ece5dd; box-shadow: 0 1px 3px rgba(47,77,58,0.05); margin-bottom: 12px; }
.dish-card-ready { border-color: #dcd0ec; background: #faf8fd; }
.dish-card-head { display: flex; align-items: center; justify-content: space-between; margin-bottom: 6px; }
.dish-mesa { font-size: 12.5px; font-weight: 700; color: #4a332b; }
.dish-time { font-size: 11.5px; color: #9ca3af; }
.dish-nombre { font-size: 15px; font-weight: 600; color: #1f2937; margin-bottom: 4px; }
.dish-nota { font-size: 12px; color: #8a6a1f; background: #faf0da; padding: 5px 8px; border-radius: 7px; margin-bottom: 8px; }
.dish-esperando { font-size: 12px; color: #6b4f8f; margin-top: 6px; }

.btn-avanzar { width: 100%; margin-top: 8px; padding: 9px; border-radius: 9px; border: none; color: #fff; font-weight: 700; font-size: 13.5px; cursor: pointer; }
.btn-avanzar-amber { background: #d99a1f; }
.btn-avanzar-amber:hover { background: #e3a92f; }
.btn-avanzar-blue { background: #3d5a80; }
.btn-avanzar-blue:hover { background: #4a6a94; }

  ...
  <div class="admin-panel">
      ...
      <div class="header-caja">
        <div>
          ...
          <div class="caja-status">
            <span
              class="dot"
              :class="caja.abierta ? 'dot-on' : 'dot-off'"
            ></span>
            <span class="caja-status-text">{{
              caja.abierta ? "Caja abierta" : "Caja cerrada"
            }}</span>
          </div>
          <div v-if="caja.abierta" class="caja-since">
            Abierta a las {{ formatTime(caja.desde) }}
          </div>
        </div>
        <div>
          <div class="label-upper">Efectivo en caja</div>
          <div class="caja-amount">
            {{ money(caja.abierta ? caja.efectivoEnCaja : 0) }}
          </div>
        </div>
        <div class="caja-cta">
          <button
            ...
            :title="
              ordenesActivas.length > 0
                ? 'Cobra o cancela las órdenes activas antes de cerrar'
                : ''
            "
          ...
          <button v-else class="btn-gold" @click="modalAbrirCaja = true">
            Abrir caja
          </button>
        </div>
      </div>
      <div
        v-if="caja.abierta && ordenesActivas.length > 0"
        class="caja-warning"
      >
        Hay órdenes activas sin cobrar — cóbralas o cancélalas antes de cerrar
        caja.
      </div>
    </header>
    ...
    <div class="admin-body">
      ...
      <aside class="admin-sidebar">
        ...
        <button
          class="nav-btn"
          :class="{ active: tab === 'ordenes' }"
          @click="setTab('ordenes')"
        >
          <span>Órdenes</span>
        </button>
      ...
      <main class="admin-main">
          ...
          <div class="panel-head">
            ...
            <div class="panel-head-actions">
              <button
                ...
                @click="abrirModalMov('entrada')"
              >
                + Ingreso
              </button>
              <button
                class="btn-mini btn-mini-red"
                :disabled="!caja.abierta"
                @click="abrirModalMov('salida')"
              >
                − Retiro
              </button>
            </div>
          </div>
          <div v-if="movimientosTurno.length === 0" class="empty-msg">
            Sin movimientos registrados aún.
          </div>
          <div v-for="m in movimientosTurno" :key="m._id" class="row-line">
            <span
              class="tag"
              :class="m.tipo === 'entrada' ? 'tag-green' : 'tag-red'"
              >{{ etiquetaMovimiento(m.tipo) }}</span
            >
            <span
              class="row-amount"
              :class="m.tipo === 'entrada' ? 'text-green' : 'text-red'"
            >
        ...
        <section v-if="tab === 'ordenes'" class="panel-stack">
          ...
          <div class="panel-card">
            <div class="panel-head">
              <div class="panel-title">
                Historial de órdenes pagadas del turno
              </div>
            </div>
            <div v-if="!caja.abierta" class="empty-msg">
              Abre la caja para ver el historial del turno.
            </div>
            <div v-else-if="ordenesPagadasTurno.length === 0" class="empty-msg">
              Aún no hay órdenes cobradas.
            </div>
            <div
              v-else
              v-for="o in ordenesPagadasTurno"
              :key="o._id"
              class="row-block"
            >
              <div
                class="row-line row-line-click"
                @click="toggleExpandirHistorial(o._id)"
              >
                <span class="row-mesa-sm"
                  >Mesa {{ o.mesa ? o.mesa.numero : "—" }}</span
                >
                <span class="row-mesero-sm">{{
                  o.mesero ? o.mesero.nombre : "—"
                }}</span>
                ...
                <span
                  class="row-chevron"
                  :class="{ open: historialExpandido[o._id] }"
                  >▾</span
                >
              </div>

              <div v-if="historialExpandido[o._id]" class="row-summary">
                <div
                  v-for="(g, i) in platillosAgrupados(o)"
                  :key="i"
                  class="row-summary-item"
                >
                  <span class="row-summary-name"
                    >{{ g.qty }}× {{ g.nombre }}</span
                  >
                  <span class="row-summary-price"
                    >{{ money(g.precioUnitario) }} c/u ·
                    {{ money(g.precioUnitario * g.qty) }}</span
                  >
                </div>
                <div v-if="o.nota" class="row-summary-nota">
                  Nota: {{ o.nota }}
                </div>
              </div>
            </div>
        ...
        <section v-if="tab === 'menu'" class="panel-card">
          <div class="panel-head">
            ...
            <button
              class="btn-mini btn-mini-outline"
              @click="abrirNuevaCategoria"
            >
              + Nueva categoría
            </button>
            <button class="btn-mini btn-mini-green" @click="abrirNuevoPlatillo">
              + Nuevo platillo
            </button>
          </div>
          <div
            v-for="cat in menuPorCategoria"
            :key="cat.nombre"
            class="menu-cat-block"
          >
            ...
            <div v-for="p in cat.items" :key="p._id" class="row-line">
              ...
              <button
                class="btn-mini btn-mini-outline"
                @click="abrirEditarPlatillo(p)"
              >
                Editar
              </button>
              <button
                class="btn-mini btn-mini-outline-red"
                @click="pedirEliminar('platillo', p)"
              >
                Eliminar
              </button>
            </div>
        ...
        <section v-if="tab === 'usuarios'" class="panel-card">
          <div class="panel-head">
            ...
            <button class="btn-mini btn-mini-green" @click="abrirNuevoUsuario">
              + Nuevo usuario
            </button>
          </div>
          <div v-for="u in usuarios" :key="u._id" class="row-line">
            ...
            <span class="tag" :class="'tag-rol-' + u.rol">{{
              etiquetaRol(u.rol)
            }}</span>
            <button
              class="btn-mini btn-mini-outline"
        ...
        <section v-if="tab === 'mesas'" class="panel-card">
          <div class="panel-head">
            ...
            <button class="btn-mini btn-mini-green" @click="abrirNuevaMesa">
              + Agregar mesa
            </button>
          </div>
    ...
    <div
      v-if="modalAbrirCaja"
      class="modal-overlay"
      @click.self="modalAbrirCaja = false"
    >
      <div class="modal-box">
        <div class="modal-title">Abrir caja</div>
        <label class="field-label">Fondo inicial de efectivo</label>
        <input
          v-model="formAbrirCaja.monto"
          class="field-input"
          placeholder="0.00"
        />
        <div class="modal-actions">
          <button class="btn-outline" @click="modalAbrirCaja = false">
            Cancelar
          </button>
          <button class="btn-solid-green" @click="confirmarAbrirCaja">
            Abrir caja
          </button>
        </div>
      </div>
    ...
    <!-- CORTE DE CAJA -->
    <div
      v-if="modalCierreCaja"
      class="modal-overlay"
      @click.self="modalCierreCaja = false"
    >
      <div class="modal-box">
        <div class="modal-title">Corte de caja</div>
        <div class="summary-line">
          <span>Fondo inicial</span><b>{{ money(caja.base) }}</b>
        </div>
        <div class="summary-line">
          <span>Ventas del turno</span><b>{{ money(caja.ventas) }}</b>
        </div>
        <div class="summary-line">
          <span>Ingresos de caja</span
          ><b class="text-green">+{{ money(caja.entradas) }}</b>
        </div>
        <div class="summary-line">
          <span>Retiros de caja</span
          ><b class="text-red">−{{ money(caja.salidas) }}</b>
        </div>
        <div class="summary-line summary-total">
          <span>Efectivo esperado</span><b>{{ money(caja.efectivoEnCaja) }}</b>
        </div>
        <div class="modal-actions">
          <button class="btn-outline" @click="modalCierreCaja = false">
            Volver
          </button>
          <button class="btn-solid-dark" @click="confirmarCierreCaja">
            Confirmar cierre
          </button>
        </div>
      </div>
    ...
    <div
      v-if="modalPlatillo"
      class="modal-overlay"
      @click.self="modalPlatillo = false"
    >
      <div class="modal-box">
        <div class="modal-title-serif">
          {{ formPlatillo._id ? "Editar platillo" : "Nuevo platillo" }}
        </div>
        <label class="field-label">Nombre del platillo</label>
        <input
          v-model="formPlatillo.nombre"
          class="field-input"
          placeholder="Ej. Tacos al pastor"
        />
        <label class="field-label">Descripción</label>
        <input
          v-model="formPlatillo.descripcion"
          class="field-input"
          placeholder="Ej. Con piña y cilantro"
        />
        <label class="field-label">Precio</label>
        <input
          v-model="formPlatillo.precio"
          class="field-input"
          placeholder="0.00"
        />
        <label class="field-label">Categoría</label>
        <input
          v-model="formPlatillo.categoria"
          class="field-input"
        ...
        <div v-if="formPlatillo.error" class="field-error">
          {{ formPlatillo.error }}
        </div>
        <div class="modal-actions">
          <button class="btn-outline" @click="modalPlatillo = false">
            Cancelar
          </button>
          <button class="btn-solid-green" @click="guardarPlatillo">
            Guardar
          </button>
        </div>
      </div>
    ...
    >
      <div class="modal-box">
        <div class="modal-title-serif">
          {{ formUsuario._id ? "Editar usuario" : "Nuevo usuario" }}
        </div>
        ...
        <input
          ...
          class="field-input"
          :placeholder="
            formUsuario._id ? 'Dejar en blanco para no cambiarla' : '••••'
          "
        />
        ...
        <div v-if="formUsuario.error" class="field-error">
          {{ formUsuario.error }}
        </div>
        <div class="modal-actions">
          <button class="btn-outline" @click="modalUsuario = false">
            Cancelar
          </button>
          <button class="btn-solid-green" @click="guardarUsuario">
            Guardar
          </button>
        </div>
      </div>
    ...
    <div v-if="modalMesa" class="modal-overlay" @click.self="modalMesa = false">
      <div class="modal-box">
        ...
        <input
          v-model="formMesa.numero"
          class="field-input"
          placeholder="Ej. 15"
        />
        <label class="field-label">Capacidad (personas)</label>
        <input
          v-model="formMesa.capacidad"
          class="field-input"
          placeholder="Ej. 4"
        />
        <div v-if="formMesa.error" class="field-error">
          {{ formMesa.error }}
        </div>
        <div class="modal-actions">
          <button class="btn-outline" @click="modalMesa = false">
            Cancelar
          </button>
    ...
    >
      <div class="modal-box">
        ...
        <input
          ...
          class="field-input"
          placeholder="Ej. Refrescos, aguas y cervezas"
        />
        <div v-if="formCategoria.error" class="field-error">
          {{ formCategoria.error }}
        </div>
        <div class="modal-actions">
          <button class="btn-outline" @click="modalCategoria = false">
            Cancelar
          </button>
          <button class="btn-solid-green" @click="guardarCategoria">
            Agregar
          </button>
        </div>
      </div>
    ...
    >
      <div class="modal-box modal-center">
        ...
        <div class="modal-title-serif">
          ¿Eliminar {{ modalEliminar.label }}?
        </div>
        <div class="modal-sub">Esta acción no se puede deshacer.</div>
        <div class="modal-actions">
          <button class="btn-outline" @click="modalEliminar = null">
            Volver
          </button>
          <button class="btn-solid-red" @click="confirmarEliminar">
            Sí, eliminar
          </button>
        </div>
      </div>
    ...
    <div
      v-if="modalCobro"
      class="modal-overlay"
      @click.self="modalCobro = null"
    >
      <div class="modal-box ticket-box">
        <div id="ticket-imprimible" class="ticket">
          <div class="ticket-brand">Cantina Central</div>
          <div class="ticket-sub">Ticket de cobro</div>
          <div class="ticket-divider"></div>
          <div class="ticket-line">
            <span>Mesa</span
            ><span>{{ modalCobro.mesa ? modalCobro.mesa.numero : "—" }}</span>
          </div>
          <div class="ticket-line">
            <span>Mesero</span
            ><span>{{
              modalCobro.mesero ? modalCobro.mesero.nombre : "—"
            }}</span>
          </div>
          <div class="ticket-line">
            <span>Fecha</span><span>{{ formatTime(new Date()) }}</span>
          </div>
          <div class="ticket-divider"></div>
          <div
            v-for="(g, i) in platillosAgrupados(modalCobro)"
            :key="i"
            class="ticket-item"
          >
            <span class="ticket-item-name">{{ g.qty }}× {{ g.nombre }}</span>
            <span class="ticket-item-price">{{
              money(g.precioUnitario * g.qty)
            }}</span>
          </div>
          <div class="ticket-divider"></div>
          <div class="ticket-line ticket-total">
            <span>Total</span><span>{{ money(modalCobro.total) }}</span>
          </div>
        </div>
        <div class="modal-actions no-print">
          <button class="btn-outline" @click="modalCobro = null">
            Cancelar
          </button>
          <button class="btn-solid-green" @click="confirmarTicketCobro">
            Confirmar cobro
          </button>
        </div>
        <button class="btn-print no-print" @click="imprimirTicket">
          Imprimir ticket
        </button>
      </div>
    </div>
    ...
    <div
      ...
      class="modal-overlay"
      @click.self="modalCancelacion = null"
    >
      <div class="modal-box ticket-box">
        <div id="ticket-imprimible" class="ticket ticket-cancelacion">
          <div class="ticket-brand">Cantina Central</div>
          <div class="ticket-sub">Ticket de cancelación</div>
          <div class="ticket-divider"></div>
          <div class="ticket-line">
            <span>Mesa</span
            ><span>{{
              modalCancelacion.mesa ? modalCancelacion.mesa.numero : "—"
            }}</span>
          </div>
          <div class="ticket-line">
            <span>Mesero</span
            ><span>{{
              modalCancelacion.mesero ? modalCancelacion.mesero.nombre : "—"
            }}</span>
          </div>
          <div class="ticket-line">
            <span>Fecha</span><span>{{ formatTime(new Date()) }}</span>
          ...
          <div class="ticket-divider"></div>
          <div
            v-for="(g, i) in platillosAgrupados(modalCancelacion)"
            :key="i"
            class="ticket-item"
          >
            <span class="ticket-item-name">{{ g.qty }}× {{ g.nombre }}</span>
          ...
          <div class="ticket-line ticket-total">
            ...
            ><span>{{ money(modalCancelacion.total) }}</span>
          </div>
          <div class="ticket-warning">
            Esta orden se cancelará y no se cobrará. Esta acción no se puede
            deshacer.
          </div>
        </div>
        <div class="modal-actions no-print">
          <button class="btn-outline" @click="modalCancelacion = null">
            Volver
          </button>
          <button class="btn-solid-red" @click="confirmarTicketCancelacion">
            Sí, cancelar orden
          </button>
        </div>
      </div>
...
export default {
  ...
  data() {
    return {
      ...
      // controla qué filas del historial de órdenes pagadas están desplegadas
      historialExpandido: {},
      ...
      modalCobro: null, // orden pendiente de mostrar en el ticket de cobro
      modalCancelacion: null, // orden pendiente de mostrar en el ticket de cancelación
      ...
      return this.movimientos
        .filter(
          (m) =>
            ...
            new Date(m.createdAt).getTime() >= desde,
        )
  ...
  created() {
    ...
    this.cargarCaja();
    this.cargarOrdenes();
    // el desglose de precios necesita el menú cargado, aunque el tab activo sea "ordenes"
    this.cargarMenu();
  },
  ...
  methods: {
    ...
    async cargarMovimientos() {
      try {
        this.movimientos = await this.peticion("/movimientos-caja");
      } catch (e) {
        this.showToast(e.message);
      }
    },
    async confirmarAbrirCaja() {
      ...
      if (isNaN(monto) || monto < 0) {
        this.showToast("Ingresa un fondo inicial válido.");
        return;
      }
      try {
    ...
    pedirCierreCaja() {
      if (this.ordenesActivas.length > 0) {
        this.showToast(
          "No puedes cerrar caja: hay órdenes activas sin cobrar.",
        );
        return;
      }
    ...
    async confirmarMov() {
      ...
      if (isNaN(monto) || monto <= 0) {
        this.formMov.error = "Ingresa un monto válido.";
        return;
      }
      if (this.formMov.tipo === "salida" && monto > this.caja.efectivoEnCaja) {
        this.formMov.error = "El retiro excede el efectivo disponible.";
        return;
      }
      try {
        await this.peticion("/movimientos-caja", {
          method: "POST",
          body: JSON.stringify({
            tipo: this.formMov.tipo,
            monto,
            cajero: this.currentUser?.id,
            descripcion: this.formMov.descripcion,
          }),
        });
        this.modalMov = false;
        this.showToast(
          this.formMov.tipo === "entrada"
            ? "Ingreso registrado."
            : "Retiro registrado.",
        );
        await this.cargarCaja();
      } catch (e) {
        this.formMov.error = e.message;
      }
    },
    etiquetaMovimiento(tipo) {
      return tipo === "entrada" ? "Ingreso" : "Retiro";
    },
    ...
    // desglose con precio unitario, usado en el historial de órdenes pagadas
    platillosAgrupados(o) {
      const grupos = {};
      o.platillos.forEach((p) => {
        const key = `${p.nombre}|${p.estado}|${p.entregado}`;
        if (!grupos[key]) {
          // usa el precio guardado en el platillo (el que tenía al momento de ordenar);
          // si no viene en la respuesta, cae al precio actual del menú como respaldo
          const menuRef = typeof p.menu === "object" ? p.menu?._id : p.menu;
          const precioUnitario =
            p.precio ?? this.menu.find((m) => m._id === menuRef)?.precio ?? 0;
          grupos[key] = {
            nombre: p.nombre,
            estado: p.estado,
    ...
    toggleExpandirHistorial(id) {
      this.historialExpandido = {
        ...this.historialExpandido,
        [id]: !this.historialExpandido[id],
      };
    },
    etiquetaEstadoOrden(o) {
      const estados = o.platillos.map((p) => p.estado);
      if (estados.every((e) => e === "ventanilla")) return "En ventanilla";
      if (estados.some((e) => e === "cocinado" || e === "ventanilla"))
        return "Cocinado";
      return "En proceso";
    },
    abrirTicketCobro(o) {
    ...
    async confirmarTicketCobro() {
      const o = this.modalCobro;
      if (!o) return;
      try {
        ...
        this.modalCobro = null;
        await Promise.all([this.cargarOrdenes(), this.cargarCaja()]);
      } catch (e) {
        this.showToast(e.message);
      }
    },
    abrirTicketCancelacion(o) {
      this.modalCancelacion = o;
    },
    async confirmarTicketCancelacion() {
      const o = this.modalCancelacion;
      if (!o) return;
      try {
        await this.peticion(`/ordenes/${o._id}`, { method: "DELETE" });
        this.showToast("Orden cancelada.");
        this.modalCancelacion = null;
    ...
    async cargarMenu() {
      try {
        this.menu = await this.peticion("/menu");
      } catch (e) {
        this.showToast(e.message);
      }
    },
    async cargarCategorias() {
      try {
        this.categorias = await this.peticion("/categorias");
      } catch (e) {
        this.showToast(e.message);
      }
    },
    ...
    async guardarCategoria() {
      ...
      try {
        ...
        if (!creada?.categoria?._id) {
          this.formCategoria.error =
            "El servidor no regresó la categoría creada.";
          return;
        }
        this.categorias.push(creada.categoria);
    ...
    async guardarPlatillo() {
      ...
      if (
        ...
        this.formPlatillo.error =
          "Completa nombre, descripción, categoría y un precio válido.";
        return;
      }
      try {
        ...
        if (this.formPlatillo._id) {
          await this.peticion(`/menu/${this.formPlatillo._id}`, {
            method: "PUT",
            body: JSON.stringify(payload),
          });
          this.showToast("Platillo actualizado.");
        } else {
          await this.peticion("/menu", {
            method: "POST",
            body: JSON.stringify(payload),
          });
        ...
        this.formPlatillo.error = e.message;
      }
    },
    ...
    async cargarUsuarios() {
      try {
        this.usuarios = await this.peticion("/usuarios");
      } catch (e) {
        this.showToast(e.message);
      }
    },
    etiquetaRol(rol) {
      return (
        { mesero: "Mesero", cocinero: "Cocinero", cajero: "Administrador" }[
          rol
        ] || rol
      );
    },
    ...
    async guardarUsuario() {
      ...
      if (!nombre || !usuario) {
        this.formUsuario.error = "Completa nombre y usuario.";
        return;
      }
      if (!this.formUsuario._id && !this.formUsuario.password) {
        this.formUsuario.error =
      ...
      try {
        if (this.formUsuario._id) {
          ...
          if (this.formUsuario.password)
            payload.password = this.formUsuario.password;
          await this.peticion(`/usuarios/${this.formUsuario._id}`, {
            method: "PUT",
            body: JSON.stringify(payload),
          });
        ...
        this.formUsuario.error = e.message;
      }
    },
    ...
    async cargarMesas() {
      try {
        this.mesas = await this.peticion("/mesas");
      } catch (e) {
        this.showToast(e.message);
      }
    },
    ...
    async guardarMesa() {
      ...
      if (!numero || numero <= 0) {
        this.formMesa.error = "Ingresa un número de mesa válido.";
        return;
      }
      if (!capacidad || capacidad <= 0) {
        this.formMesa.error = "Ingresa una capacidad válida.";
        return;
      }
      if (this.mesas.some((t) => t.numero === numero)) {
        this.formMesa.error = "Ya existe una mesa con ese número.";
        return;
      }
      try {
        await this.peticion("/mesas", {
          method: "POST",
    ...
    pedirEliminar(kind, item) {
      const labels = {
        ...
        usuario: item.nombre,
        mesa: "Mesa " + item.numero,
      };
      const paths = {
        platillo: `/menu/${item._id}`,
        usuario: `/usuarios/${item._id}`,
        mesa: `/mesas/${item._id}`,
      };
...
.panel-card {
  ...
  border-radius: 14px;
  padding: 18px 20px;
  box-shadow: 0 1px 3px rgba(74, 51, 43, 0.06);
...
/* ---------- historial expandible con desglose ---------- */
.row-block {
  border-bottom: 1px solid #f1f5f9;
}
...
.btn-mini-outline-red:hover:not(:disabled) {
  background: #faeee9;
}
...
.ticket-cancelacion .ticket-total {
...
@media print {
  ...
  /* oculta todo el panel de administración: header, sidebar y contenido */
  .admin-header,
  .admin-body,
  .toast {
  ...
  /* el overlay del modal deja de ser pantalla completa y pasa a fluir normal */
  .modal-overlay {

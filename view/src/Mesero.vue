<template>
  <div class="mesero-panel">
    ...
    <header class="mesero-header">
      <div class="header-top">
        <div class="brand">
          ...
          <div class="user-pill">
            Mesero · {{ currentUser ? currentUser.nombre : "" }}
          </div>
        </div>
        <div class="header-actions">
          <div class="bell-wrap">
            <button class="btn-bell" @click="toggleBell">
              🔔
              <span v-if="campanitaCount > 0" class="bell-badge">{{
                campanitaCount
              }}</span>
            </button>
            <div v-if="showBellDropdown" class="bell-dropdown">
              <div class="bell-title">Listas en ventanilla</div>
              <div v-if="platillosListos.length === 0" class="empty-msg-sm">
                Nada pendiente por ahora.
              </div>
              <div
                v-for="(item, i) in platillosListos"
                :key="i"
                class="bell-item"
              >
    ...
    <div v-if="!caja.abierta" class="banner-warning">
      La caja está cerrada — no se pueden tomar nuevas órdenes hasta que el
      administrador la abra.
    </div>
    ...
    <div v-if="tab === 'tablero'" class="content">
      <div v-if="ordenesActivas.length === 0" class="empty-msg">
        No hay órdenes activas en el restaurante.
      </div>
      <div class="orders-grid">
        <div v-for="o in ordenesActivas" :key="o._id" class="order-card">
          <div class="order-card-head">
            <div class="order-mesa-title">
              Mesa {{ o.mesa ? o.mesa.numero : "—" }}
            </div>
            <span class="tag" :class="estadoOrden(o).clase">{{
              estadoOrden(o).label
            }}</span>
          </div>
          ...
          <div class="dish-list">
            ...
            >
              ...
              <span class="tag tag-sm" :class="dishEstadoClase(g)">{{
                dishEstadoLabel(g)
              }}</span>
              <button
          ...
          <div class="order-card-foot">
            <div class="order-meta">
              {{ formatTime(o.createdAt) }} · {{ money(o.total) }}
            </div>
            <div v-if="!esMiOrden(o)" class="tag tag-waiter">
              {{ o.mesero ? o.mesero.nombre : "—" }}
            </div>
          </div>

          <div v-if="todoEntregado(o)" class="order-delivered">
            ✓ Orden entregada
          </div>
          <button
    ...
    <div v-if="tab === 'pagadas'" class="content">
      <div v-if="ordenesPagadasTurno.length === 0" class="empty-msg">
        Aún no hay órdenes pagadas en este turno.
      </div>
      <div class="panel-card" v-else>
        <div v-for="o in ordenesPagadasTurno" :key="o._id" class="row-block">
          <div
            class="row-line row-line-click"
            @click="toggleExpandirPagada(o._id)"
          >
            ...
            <span
              class="row-chevron"
              :class="{ open: pagadasExpandidas[o._id] }"
              >▾</span
            >
          </div>

          <div v-if="pagadasExpandidas[o._id]" class="row-summary">
            <div
              v-for="(g, i) in platillosAgrupados(o)"
              :key="i"
              class="row-summary-item"
            >
              <span class="row-summary-name">{{ g.qty }}× {{ g.nombre }}</span>
              <span class="row-summary-price"
                >{{ money(g.precioUnitario) }} c/u ·
                {{ money(g.precioUnitario * g.qty) }}</span
              >
            </div>
            <div v-if="o.nota" class="row-summary-nota">Nota: {{ o.nota }}</div>
          </div>
        </div>
      </div>
    </div>
      ...
      <div class="modal-box modal-wide">
        <div class="modal-head">
          <div class="modal-title-serif">
            {{ formOrden._id ? "Editar orden" : "Nueva orden" }}
          </div>
        ...
        <template v-else>
          ...
          <div class="field-hint">
            Los platillos que agregues aquí se envían como una nueva ronda a
            cocina.
          </div>
        </template>
        ...
        <div class="menu-picker">
          ...
          >
            ...
            <div v-for="p in cat.items" :key="p._id" class="qty-row">
              ...
              <button
                class="btn-qty"
                :disabled="(formOrden.qtys[p._id] || 0) <= minQty(p._id)"
                @click="decQty(p._id)"
              >
                −
              </button>
        ...
        <div v-if="formOrden.error" class="field-error">
          {{ formOrden.error }}
        </div>

        <div class="modal-foot">
          <div class="modal-foot-total">
            {{ draftItemCount }} platillos · <b>{{ money(draftTotal) }}</b>
          </div>
          <button class="btn-solid-gold" @click="guardarOrden">
            Enviar a cocina →
          </button>
        </div>
      </div>
...
export default {
  ...
  data() {
    return {
      ...
      // controla qué filas de "Pagadas del turno" están desplegadas mostrando su resumen
      pagadasExpandidas: {},
  ...
  methods: {
    ...
    esMiOrden(o) {
      return !!(
        o.mesero &&
        this.currentUser &&
        o.mesero._id === this.currentUser.id
      );
    },
    todoEntregado(o) {
      ...
      return (
        {
          proceso: "tag-amber",
          cocinado: "tag-status",
          ventanilla: "tag-purple",
        }[g.estado] || "tag-status"
      );
    },
    estadoOrden(o) {
      if (this.todoEntregado(o))
        return { label: "Entregada", clase: "tag-green" };
      const estados = o.platillos.map((p) => p.estado);
      if (estados.every((e) => e === "ventanilla"))
        return { label: "En ventanilla", clase: "tag-purple" };
      if (estados.some((e) => e === "cocinado" || e === "ventanilla"))
        return { label: "En proceso", clase: "tag-status" };
    ...
    async entregarGrupo(o, g) {
      try {
        await Promise.all(
          g.ids.map((id) =>
            this.peticion(`/ordenes/${o._id}/platillos/${id}/entregar`, {
              method: "PUT",
            }),
          ),
        );
    ...
    // ---------- pagadas del turno ----------
    toggleExpandirPagada(id) {
      this.pagadasExpandidas = {
        ...this.pagadasExpandidas,
        [id]: !this.pagadasExpandidas[id],
      };
    },

    // ---------- mesas ----------
    mesaOcupada(t) {
      return this.ordenesActivas.some(
        (o) => o.mesa && o.mesa._id === t._id && o._id !== this.formOrden._id,
      );
    },
    ...
    abrirNuevaOrden() {
      if (!this.caja.abierta) {
        this.showToast("No puedes tomar órdenes: la caja está cerrada.");
        return;
      }
      this.formOrden = {
        _id: null,
    ...
    abrirEditarOrden(o) {
      const snapshot = {};
      o.platillos.forEach((p) => {
        snapshot[p.menu] = (snapshot[p.menu] || 0) + 1;
      });
      this.formOrden = {
        _id: o._id,
        mesaId: o.mesa ? o.mesa._id : "",
        mesaNumero: o.mesa ? o.mesa.numero : "",
        qtys: { ...snapshot },
        snapshotQtys: snapshot,
        nota: o.nota || "",
        error: "",
      };
      this.modalOrden = true;
    ...
    async guardarOrden() {
      ...
      if (entries.length === 0) {
        this.formOrden.error = this.formOrden._id
          ? "Agrega al menos un platillo más."
      ...
      try {
        if (this.formOrden._id) {
          ...
          if (!this.formOrden.mesaId) {
            this.formOrden.error = "Selecciona una mesa.";
            return;
          }
          await this.peticion("/ordenes", {
            method: "POST",
            body: JSON.stringify({
              mesa: this.formOrden.mesaId,
              mesero: this.currentUser?.id,
              platillos,
              nota: this.formOrden.nota,
            }),
          });
        ...
        this.formOrden.error = e.message;
      }
    },
  },
};
...
/* ---------- pagadas (fila expandible con resumen de platillos) ---------- */
.panel-card {
  background: #fff;
  border-radius: 14px;
  padding: 8px 20px;
